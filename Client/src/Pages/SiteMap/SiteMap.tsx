...
const SiteMap: React.FC = () => {
  ...
  return (
    <section className="max-w-5xl mx-auto px-4 py-12">
      <div className="bg-surface border border-accent rounded-2xl shadow-lg p-8">
        ...
        <ul className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {links.map((link, index) => (
            <li key={index}>
              <a
                href={link.path}
                className="block bg-background text-accent font-medium text-sm text-center py-2 px-3 rounded-md shadow hover:bg-accent hover:text-background transition duration-300"
              >
                {link.name}
              </a>
            </li>
